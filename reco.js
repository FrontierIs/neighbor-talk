  ...
  //counterの値、取得して+1して保存
   ...
   if (preflist[keys[keys.length-1]]-preflist[keys[0]]>=3 || counter>=3 ){
      ...
      if (negalist.indexOf(array[0]) == -1){
        console.log(items)
                if (counter==4){
                  start=setTimeout(function(){
                    ipcRenderer.sendToHost('hover',"sh;clear\n")
                    ipcRenderer.sendToHost('hover',"sh;list2\n")
                    console.log("list送った")
                  },2500)
                }

                else if(items[show][8]==1 && items[show][0]==1){
                  start=setTimeout(function(){
                    ipcRenderer.sendToHost('hover',"sh;clear\n")
                    ipcRenderer.sendToHost('hover',"sh;target\n")
                    console.log("target送った")
                    setTimeout(function(){
                ...
                //   start=setTimeout(function(){
                //     ipcRenderer.sendToHost('hover',"sh;clear\n")
                //     ipcRenderer.sendToHost('hover',"sh;ok_heart\n")
                //     console.log("ハート送った")
                //     setTimeout(function(){
                //       ipcRenderer.sendToHost("hover","sh;boost\n")
                //     },35000)
                ...
                //   start=setTimeout(function(){
                //     ipcRenderer.sendToHost('hover',"sh;clear\n")
                //     ipcRenderer.sendToHost('hover',"sh;ok_shizuku\n")
                //     console.log("しずく送った")
                //     setTimeout(function(){
                //       ipcRenderer.sendToHost("hover","sh;boost\n")
                //     },35000)
                ...
                //   start=setTimeout(function(){
                //     ipcRenderer.sendToHost('hover',"sh;clear\n")
                //     ipcRenderer.sendToHost('hover',"sh;ok_gold\n")
                //     console.log("gold送った")
                //     setTimeout(function(){
                //       ipcRenderer.sendToHost("hover","sh;boost\n")
                //     },35000)
                ...
                // }
                  ...
                  start=setTimeout(function(){
                    ...
                    console.log("long送った")
                          ...
                          p=Math.floor(Math.random() * (talktitle.length - 0))
                          if(p%2==0){
                            setTimeout(function(){
                              ipcRenderer.sendToHost('hover',"sh;clear\n")
                              ipcRenderer.sendToHost('hover', "sh;praiselong\n")
                            ...
                            setTimeout(function(){
                              ipcRenderer.sendToHost('hover',"sh;clear\n")
                              ipcRenderer.sendToHost('hover', "sh;praisesilver\n")
                            },2000)
                          }
              }
      }
        ...
        setTimeout(function(){
          ipcRenderer.sendToHost('hover',"sh;clear\n")
          ipcRenderer.sendToHost('hover', "sh;dis\n")
        },2000)
      }
    ...
    //詳細ページでsuggestモードではないとき
      ...
      else if(items[show][9]==1 && items[show][1]==1){
        start=setTimeout(function(){
          ipcRenderer.sendToHost('hover',"sh;clear\n")
          ipcRenderer.sendToHost('hover',"sh;target\n")
          console.log("silver送った")
