...
public class Config extends FileManage{
	...
	public Config(String configName){
		...
		if(!init(configName))
			new Log("Can't initialize 'Config.java'",1);
	...
	public Config(String configName, String folder){
		...
		if(str=="/"){
			...
			new Log("config foloder name error : " + folder);
			FILEDIR = folder + "/";
		}
	...
	public boolean Set(String key,Object content){
		...
		if(i==configArray.size()){
			...
			if(str.equals(String.valueOf(configArray.get(i)))){
				new Log("Not change config " + str, false);
			}else{
				new Log("change config   FROM: " + String.valueOf(configArray.get(i)) + "   TO: " + str);
