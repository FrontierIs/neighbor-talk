...
public class Log extends FileManage{
	...
	static String LOGNUM = "_logNum";
	static int currentNum = 0;
	static int maxCurrentNum = 100;
	...
	private boolean init(Throwable trace){
		...
		synchronized (lock){
			...
			try{
					...
					File logNum = new File(LOGDIR + "/" + LOGFILE + LOGNUM + ".log");
    ...
    private class  Log_Thread extends Thread{
    	...
    	public void run(){
			...
			if(!init(trace))
				new Log("Can't initialize 'Log.java'",1);

	    	if(startDeep>endDeep){
	    		new Log("Log set ERORR startDeep > endDeep :" + startDeep + " > " + endDeep,2);
	    		startDeep = endDeep;
	    	}
				...
				while(!IsAvailable(file)){
	 			...
	 			for(int i=startDeep; i<=endDeep; i++){
					...
					if(i==startDeep){
	 				...
	 				if(systemOut){
	 					//String temp = className.substring(className.indexOf(".")+1);
